...
// GenerateHelmRepoEnv provides a mock function.
func (_m *MockEnvResolver) GenerateHelmRepoEnv(orgName string) environment.EnvSettings {
	ret := _m.Called(orgName)

	var r0 environment.EnvSettings
	if rf, ok := ret.Get(0).(func(string) environment.EnvSettings); ok {
		r0 = rf(orgName)
	} else {
		r0 = ret.Get(0).(environment.EnvSettings)
	}
